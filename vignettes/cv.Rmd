...
predict(rf_workflow %>% fit(data = penguins_train), 
        new_data = penguins_test)
```


# Translating all of these into a CV framework

```{r}
## Note! This is a tibble object
penguins_cv = rsample::vfold_cv(penguins, v = 5, repeats = 10)
penguins_cv

## Being lazy here and use the full data to make a recipe
penguins_rec = recipe(species ~ ., data = penguins) %>% 
  step_knnimpute(all_predictors())
...
tidy_rf = function(model_fit, new_data){
  new_data %>% 
    bind_cols(
...
penguins_out = penguins_cv %>% 
  dplyr::mutate(
    fit = purrr::map(
      .x = splits, 
      .f = ~ fit(penguins_workflow, data = analysis(.x))),
    tidy_out = purrr::map2(
      .x = fit, 
      .y = splits,
      .f = ~ tidy_rf(model_fit = .x, new_data = assessment(.y)))
  )

penguins_plotdf = penguins_out %>% 
  dplyr::transmute(
    id, id2, 
    purrr::map_dfr(.x = tidy_out,
                    .f = ~ f_meas(data = .x, 
                                  truth = species, 
                                  estimate = .pred_class)))
